...
impl PollardNode {
    ...
    fn new(hash: NodeHash, remember: bool) -> Rc<PollardNode> {
        Rc::new(PollardNode {
            remember,
            hash: Cell::new(hash),
    ...
    fn children(&self) -> (Option<Rc<PollardNode>>, Option<Rc<PollardNode>>) {
        if self.aunt().is_none() {
            return (self.left_niece(), self.right_niece());
        }
    ...
    fn sibling(&self) -> Option<Rc<PollardNode>> {
        let aunt = self.aunt()?;
    ...
    fn aunt(&self) -> Option<Rc<PollardNode>> {
    ...
    fn grandparent(&self) -> Option<Rc<PollardNode>> {
        self.aunt()?.aunt()
    }
    ...
    /// node.
    fn swap_nieces(&self, other: &PollardNode) {
        std::mem::swap(
    ...
    fn left_niece(&self) -> Option<Rc<PollardNode>> {
    ...
    ///
    /// If this node is a leaf, this function should return `None`, as leaves don't have nieces.
    fn right_niece(&self) -> Option<Rc<PollardNode>> {
...
pub struct Pollard {
    ...
    roots: [Option<Rc<PollardNode>>; 64],
...
impl Pollard {
    ...
    pub fn batch_proof(&self, targets: &[u64]) -> Result<Proof, &'static str> {
    ...
    pub fn prove_single(&self, pos: u64) -> Result<Proof, &'static str> {
...
type AddSingleResult = (Vec<(u64, NodeHash)>, Vec<usize>);
...
impl Pollard {
    ...
    fn ingest_positions(
        &mut self,
        mut iter: impl Iterator<Item = (u64, NodeHash)>,
    ) -> Result<(), String> {
    ...
    fn prove_single_inner(&self, pos: u64) -> Result<Vec<NodeHash>, &'static str> {
    ...
    fn add_single(
        &mut self,
        node: PollardAddition,
    ) -> Result<AddSingleResult, String> {
        let mut row = 0;
    ...
    fn delete_single(&mut self, node: Rc<PollardNode>) -> Result<(), String> {
